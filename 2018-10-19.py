...
s1 >> orient(
    ...
    amp=PWhite(0.5, 1) * expvar([0.5, 1], 0.5) * 1,
...
d1 >> play(
    "--*-", sample=2, dur=0.5, pan=PWhite(-0.8, 0.8), amp=PWhite(0.8, 1.2), rate=P[1,-1,1,1,.2].shuffle()
).sometimes("amen")

b1 >> sawbass(s2.degree, sus=0.3, cutoff=900, dur=PDur(5, 8), amp=expvar([0.5, 1], 0.5))

k1 >> play("X ", sample=5, shape=(0,1), room=0, mix=0, echo=0, amp=3.0).sometimes(
    "stutter", 4, dur=3
).sometimes("amen")
